...
const ListadoOrdenar = [
    ...
    {
        indice: 4, title: 'Descripción', icono: faUser
    },
    {
...
const ListadoEventosProximos = (props) => {
    ...
    useEffect(() => {
        ...
        const peticionFavoritos = axios.get(`${getBaseAdressApi()}api/visitasproximoseventos/`).then(response => {
            ...
            <div className="container-listado-eventos">
                ...
                <div className="listado-default-header">
                    {
                        ...
                        ListadoOrdenar.map((header, indice) => {
                            ...
                            return (
                                header.title != "Bandera" && header.title != "Seleccionar..." && header.title!="Descricpión" ?
                    ...
                    <div>Opciones <br /><span className="opciones-listados-small">(el icono de barras abre opciones tales como ver cada elemento)</span></div>
                </div>
                ...
                <div className="listado-default">
                    <div className='default-loader-full' style={cargaPaginada === false ? { display: 'block' } : { display: 'none' }}>
                        ...
                        <pre className="legend-loading-relatos-miniatures">Cargando listado de eventos próximos...</pre>
                    </div>
                    {
                        listado.map((item, index) => {
                            ...
                            let descripcion_evento = item.descripcion.length > 150 ? item.descripcion.substring(0,150)+"..." : item.descripcion
                            return (
                                <div className="evt-listado" key={index}>
                                    <div className="check-listado">{evtChecked === item.index ?
                                        ...
                                        {item.title}</div><div>{fechaevt}</div><div>{descripcion_evento}</div><div>{item.duracion} minutos</div><div>
...
export default ListadoEventosProximos;
